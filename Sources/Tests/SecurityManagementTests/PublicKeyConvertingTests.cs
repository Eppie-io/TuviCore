...
namespace SecurityManagementTests
{
    public class PublicKeyConvertingTests
    {
        ...
        {
            Assert.Throws<ArgumentNullException>((Action)() => _svc.Decode(null), "Email name can not be a null.");
        }
        ...
        public void ECPubKeyConvertingTooLongPubKeyThrowArgumentException(int childKeyNum)
        {
            ...
            Assert.Throws<ArgumentException>((Action)() => Base32EConverter.ToEmailBase32(publicKeyAsBytes));
        }
        ...
        {
            Assert.Throws<ArgumentException>((Action)() => _svc.Decode(emailName), "Incorrect length of email name.");
        }
        ...
        {
            Assert.Throws<FormatException>((Action)() => _svc.Decode(emailName), "Invalid point format. Encoded public key should start with 0x02 or 0x03.");
        }

        [Test]
        public void ToPublicKeyBase32ENullPublicKeyThrowsArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>((Action)() => _svc.Encode((ECPublicKeyParameters)null));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>((Action)() => _svc.DeriveEncoded(null, 0, 0, 0, 0));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>((Action)() => _svc.DeriveEncoded(null, "test-tag"));
        }
        ...
        {
            var masterKey = TestData.MasterKey;
            Assert.Throws<ArgumentException>((Action)() => _svc.DeriveEncoded(masterKey, null));
        }
        ...
        {
            var masterKey = TestData.MasterKey;
            Assert.Throws<ArgumentException>((Action)() => _svc.DeriveEncoded(masterKey, string.Empty));
        }
        ...
        public void ToPublicKeyBase32EAsyncUnsupportedNetworkThrowsArgumentException()
        {
            var publicKey = EccPgpContext.GenerateEccPublicKey(TestData.MasterKey, 0, 0, 0, 1);
            var publicKeyBase32E = _svc.Encode(publicKey);
            Assert.Throws<ArgumentException>((Action)() => EmailAddress.CreateDecentralizedAddress((NetworkType)999, publicKeyBase32E));
        }
        ...
        public void ToPublicKeyInvalidCurveOidThrowsException()
        {
            ...
            Assert.Throws<ArgumentException>((Action)() => _svc.Decode(InvalidEncodedKey));
        }
    }
}
